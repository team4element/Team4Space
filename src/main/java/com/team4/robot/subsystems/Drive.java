...
import com.team4.robot.Kinematics;
import com.team4.robot.RobotState;
import com.team4.robot.constants.GainConstants;
import com.team4.robot.constants.IOConstants;
import com.team4.robot.constants.PhysicalConstants;
import com.team4.robot.loops.ILooper;
...
public class Drive extends Subsystem {
    ...
    private void configureMaster(TalonSRX talon, boolean left) {
        ...
        talon.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_50Ms, IOConstants.kLongCANTimeoutMs);
        talon.configVelocityMeasurementWindow(1, IOConstants.kLongCANTimeoutMs);
        talon.configClosedloopRamp(GainConstants.kDriveVoltageRampRate, IOConstants.kLongCANTimeoutMs);
    ...
    private Drive() {
        ...
        mLeftMaster = TalonSRXFactory.createDefaultTalon(IOConstants.kLeftDriveMasterId);
        configureMaster(mLeftMaster, true);

        mLeftSlaveA = TalonSRXFactory.createPermanentSlaveVictor(IOConstants.kLeftDriveSlaveAId, mLeftMaster);
        mLeftSlaveA.setInverted(false);

        mLeftSlaveB = TalonSRXFactory.createPermanentSlaveVictor(IOConstants.kLeftDriveSlaveBId, mLeftMaster);
        mLeftSlaveB.setInverted(false);

        mRightMaster = TalonSRXFactory.createDefaultTalon(IOConstants.kRightDriveMasterId);
        configureMaster(mRightMaster, false);

        mRightSlaveA = TalonSRXFactory.createPermanentSlaveVictor(IOConstants.kRightDriveSlaveAId, mRightMaster);
        mRightSlaveA.setInverted(true);

        mRightSlaveB = TalonSRXFactory.createPermanentSlaveVictor(IOConstants.kRightDriveSlaveBId, mRightMaster);
    ...
    public synchronized void readPeriodicInputs() {
        ...
        double deltaLeftTicks = ((mPeriodicIO.left_position_ticks - prevLeftTicks) / IOConstants.kDriveEncoderPPR)
                * Math.PI;
        mPeriodicIO.left_distance += deltaLeftTicks * (PhysicalConstants.kDriveWheelDiameterInches * PhysicalConstants.kDriveWheelGearRatio);

        double deltaRightTicks = ((mPeriodicIO.right_position_ticks - prevRightTicks) / IOConstants.kDriveEncoderPPR)
                * Math.PI;
        mPeriodicIO.right_distance += deltaRightTicks * (PhysicalConstants.kDriveWheelDiameterInches * PhysicalConstants.kDriveWheelGearRatio);
    ...
    public synchronized void writePeriodicOutputs() {
        ...
        if(mTalonControlState == TalonControlState.OPEN || mDriveControlState == DriveControlState.OPEN_LOOP){
    ...
    private static double rotationsToInches(double rotations) {
        return rotations * (PhysicalConstants.kDriveWheelCircumferenceInches * PhysicalConstants.kDriveWheelGearRatio);
    }
    ...
    private static double inchesToRotations(double inches) {
        return inches / (PhysicalConstants.kDriveWheelGearRatio * PhysicalConstants.kDriveWheelCircumferenceInches);
    }
    ...
    private static double radiansPerSecondToTicksPer100ms(double rad_s) {
        return rad_s / (Math.PI * 2.0) * IOConstants.kDriveEncoderPPR / 10.0;
    }
    ...
    public synchronized void resetEncoders() {
        mLeftMaster.setSelectedSensorPosition(0, 0, IOConstants.kCANTimeoutMs);
        mRightMaster.setSelectedSensorPosition(0, 0, IOConstants.kCANTimeoutMs);
        mPeriodicIO = new PeriodicIO();
    }

    public double getLeftEncoderRotations() {
        return mPeriodicIO.left_position_ticks / IOConstants.kDriveEncoderPPR;
    }

    public double getRightEncoderRotations() {
        return mPeriodicIO.right_position_ticks / IOConstants.kDriveEncoderPPR;
    }
    ...
    public double getRightLinearVelocity() {
        return rotationsToInches(getRightVelocityNativeUnits() * 10.0 / IOConstants.kDriveEncoderPPR);
    }
    ...
    public double getLeftLinearVelocity() {
        return rotationsToInches(getLeftVelocityNativeUnits() * 10.0 / IOConstants.kDriveEncoderPPR);
    }
    ...
    public double getAngularVelocity() {
        return (getRightLinearVelocity() - getLeftLinearVelocity()) / PhysicalConstants.kDriveWheelTrackWidthInches;
    }
    ...
    public synchronized void setWantDrivePath(Path path, boolean reversed) {
        if (mCurrentPath != path || mDriveControlState != DriveControlState.PATH_FOLLOWING) {
            ...
            mPathFollower = new PathFollower(path, reversed, new PathFollower.Parameters(
                    new Lookahead(GainConstants.kMinLookAhead, GainConstants.kMaxLookAhead, GainConstants.kMinLookAheadSpeed,
                            GainConstants.kMaxLookAheadSpeed),
                    GainConstants.kInertiaSteeringGain, GainConstants.kPathFollowingProfileKp,
                    GainConstants.kPathFollowingProfileKi, GainConstants.kPathFollowingProfileKv,
                    GainConstants.kPathFollowingProfileKffv, GainConstants.kPathFollowingProfileKffa,
                    GainConstants.kPathFollowingProfileKs, GainConstants.kPathFollowingMaxVel,
                    GainConstants.kPathFollowingMaxAccel, GainConstants.kPathFollowingGoalPosTolerance,
                    GainConstants.kPathFollowingGoalVelTolerance, GainConstants.kPathStopSteeringDistance));
            mDriveControlState = DriveControlState.PATH_FOLLOWING;
            mCurrentPath = path;
        } else {
    ...
    public synchronized void setWantTurnToHeading(Rotation2d heading) {
        ...
        //  setPosition(new DriveSignal(mInitLeftPosition, mInitRightPosition));
         ...
         if (Math.abs(heading.inverse().rotateBy(mTargetHeading).getDegrees()) > 1E-3) {
             ...
             mInitTicksNeeded = (int)(inchesToRotations(wheel_delta.getRight()) * IOConstants.kDriveEncoderPPR) ;
    ...
    private void updatePathFollower(double timestamp) {
        if (mDriveControlState == DriveControlState.PATH_FOLLOWING) {
            ...
            if (!mPathFollower.isFinished()) {
                ...
                setVelocity(setpointInTicks, new DriveSignal(0, 0));
            } else {
    ...
    private void updateTurnToHeading(double timestamp) {
        ...
        int ticksNeeded = (int)(inchesToRotations(wheel_delta.getRight()) * IOConstants.kDriveEncoderPPR ) ;
    ...
    private void configureMagicTalon(){
        ...
        mLeftMaster.configClosedloopRamp(12/GainConstants.kDriveVoltageRampRate);
        mRightMaster.configClosedloopRamp(12/GainConstants.kDriveVoltageRampRate);
    ...
    public void reloadGains(){
        mLeftMaster.config_kP(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityKp, IOConstants.kLongCANTimeoutMs);
        mLeftMaster.config_kI(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityKi, IOConstants.kLongCANTimeoutMs);
        mLeftMaster.config_kD(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityKd, IOConstants.kLongCANTimeoutMs);
        mLeftMaster.config_kF(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityKf, IOConstants.kLongCANTimeoutMs);
        mLeftMaster.config_IntegralZone(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityIZone, IOConstants.kLongCANTimeoutMs);
  
        mRightMaster.config_kP(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityKp, IOConstants.kLongCANTimeoutMs);
        mRightMaster.config_kI(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityKi, IOConstants.kLongCANTimeoutMs);
        mRightMaster.config_kD(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityKd, IOConstants.kLongCANTimeoutMs);
        mRightMaster.config_kF(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityKf, IOConstants.kLongCANTimeoutMs);
        mRightMaster.config_IntegralZone(kHighGearVelocityControlSlot, GainConstants.kDriveLowGearVelocityIZone, IOConstants.kLongCANTimeoutMs);
  
        mLeftMaster.config_kP(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionKp, IOConstants.kLongCANTimeoutMs);
        mLeftMaster.config_kI(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionKi, IOConstants.kLongCANTimeoutMs);
        mLeftMaster.config_kD(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionKd, IOConstants.kLongCANTimeoutMs);
        mLeftMaster.config_kF(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionKf, IOConstants.kLongCANTimeoutMs);
        mLeftMaster.config_IntegralZone(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionIZone, IOConstants.kLongCANTimeoutMs);
        mLeftMaster.configMotionCruiseVelocity((int)radiansPerSecondToTicksPer100ms(GainConstants.kDriveLowGearMaxVelocity*(PhysicalConstants.kDriveWheelCircumferenceInches * PhysicalConstants.kDriveWheelGearRatio)/60));
        mLeftMaster.configMotionAcceleration((int)radiansPerSecondToTicksPer100ms(GainConstants.kDriveLowGearMaxAccel*(PhysicalConstants.kDriveWheelCircumferenceInches * PhysicalConstants.kDriveWheelGearRatio)/60));
  
  
        mRightMaster.config_kP(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionKp, IOConstants.kLongCANTimeoutMs);
        mRightMaster.config_kI(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionKi, IOConstants.kLongCANTimeoutMs);
        mRightMaster.config_kD(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionKd, IOConstants.kLongCANTimeoutMs);
        mRightMaster.config_kF(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionKf, IOConstants.kLongCANTimeoutMs);
        mRightMaster.config_IntegralZone(kLowGearPositionControlSlot, GainConstants.kDriveLowGearPositionIZone, IOConstants.kLongCANTimeoutMs);
        mRightMaster.configMotionCruiseVelocity((int)radiansPerSecondToTicksPer100ms(GainConstants.kDriveLowGearMaxVelocity*(PhysicalConstants.kDriveWheelCircumferenceInches * PhysicalConstants.kDriveWheelGearRatio)/60));
        mRightMaster.configMotionAcceleration((int)radiansPerSecondToTicksPer100ms(GainConstants.kDriveLowGearMaxAccel*(PhysicalConstants.kDriveWheelCircumferenceInches * PhysicalConstants.kDriveWheelGearRatio)/60));
